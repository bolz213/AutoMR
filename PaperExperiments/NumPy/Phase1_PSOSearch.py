...
class PSO:
    ...
    def generate_initial_B_all(self):
        B_all_cons = np.round(np.random.uniform(low=self.const_range[0], high=self.const_range[1],
                                       size=(self.shape_of_B_all[0], 1)), decimals=2)
        B_all_coeff = np.round(np.random.uniform(low=self.coeff_range[0], high=self.coeff_range[1],
                                        size=(self.shape_of_B_all[0], (self.shape_of_B_all[1] - 1))), decimals=1)
    ...
    def update_AB_all(self, i0_all, A_all, A_v_all, B_all, B_v_all, A_all_p_best, B_all_p_best, index_g_best,
        ...
        A_all_next_cons = np.round(A_all_next[:, :, :, 0:1], decimals=2)
        A_all_next_coeff = np.round(A_all_next[:, :, :, 1:], decimals=1)
        B_all_next_cons = np.round(B_all_next[:, 0:1], decimals=2)
        B_all_next_coeff = np.round(B_all_next[:, 1:], decimals=1)
